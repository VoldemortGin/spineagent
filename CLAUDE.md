...
src/agentspine/
  ...
  llm/provider.py           真实 LLM provider 适配器:AnthropicProvider(anthropic SDK,默认 claude-opus-4-8)/ OpenAICompatProvider(openai SDK + base_url 覆盖兼容端点)+ llm_providers Registry;各走可选 extra 延迟 import,离线默认仍 MockProvider,各用原生 SDK 不 shim
